...
namespace Cuts {
    ...
    // At the end of each event, we either have a tetra, penta or hexaquark (or none).
    // Regardless, we add an entry to all three branches. This helps us filter out the
    // events where no such multiquark was found
    inline TCut TetraquarkCut = "!TMath::IsNaN(recMassKK)";
    inline TCut PentaquarkCut = "!TMath::IsNaN(recMassKL)";
    inline TCut HexaquarkCut  = "!TMath::IsNaN(recMassLL)";
